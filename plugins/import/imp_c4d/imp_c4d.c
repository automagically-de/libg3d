	...
	G3DModel *model, gsize *n_bytes, guint32 *level)
{
	...
	gchar *str;
	gint32 i;
	static gchar padding_str[] = "                             "
		"                                                      ";
	gchar *padding = (padding_str + strlen(padding_str) - *level);
	...
	switch(opcode) {
		case 0x01: /* 64 bit something */
			...
			*n_bytes += 8;
			...
			g_debug("\\%s01: 0x%08x 0x%08x", padding, x1, x2);
#endif
			break;
		...
		case 0x16: /* 2 x float */
			...
			*n_bytes += 8;
#if DEBUG > C4D_DEBUG_OPCODE
			g_debug("\\%s16: %.3f, %.3f", padding, f1, f2);
#endif
			break;
		case 0x17: /* 3 x float */
			f1 = g3d_stream_read_float_be(stream);
			...
			*n_bytes += 12;
#if DEBUG > C4D_DEBUG_OPCODE
			g_debug("\\%s17: %.3f, %.3f, %.3f", padding, f1, f2, f3);
#endif
			break;
		case 0x19: /* 12 x float */
			...
			for(i = 0; i < 4; i ++) {
				...
				*n_bytes += 12;
#if DEBUG > C4D_DEBUG_OPCODE
				g_debug("\\%s    %.3f, %.3f, %.3f", padding, f1, f2, f3);
#endif
			}
			break;
		case 0x80: /* data */
		...
		case 0x81: /* embedded file */
			...
			*n_bytes += x1;
#if DEBUG > C4D_DEBUG_OPCODE
			g_debug("\\%s81: skipped %d bytes", padding, x1);
#endif
			break;
		case 0x82: /* wide-char string */
			str = c4d_read_wchar(stream, n_bytes);
#if DEBUG > C4D_DEBUG_OPCODE
			g_debug("\\%s82: %s", padding, str);
#endif
			if(str)
				g_free(str);
			break;
		...
		case 0x87: /* n * 3 * int16 */
			...
			*n_bytes += 4;
			...
			for(i = 0; i < (x1 / 6); i ++) {
				...
				*n_bytes += 6;
#if DEBUG > C4D_DEBUG_OPCODE
				g_debug("\\%s    0x%04x 0x%04x 0x%04x", padding, w1, w2, w3);
#endif
			}
			break;
		default:
			if(g3d_stream_eof(stream))
				return TRUE;
			g_debug("%sunknown opcode 0x%02X @ 0x%08x", padding, opcode,
	...
	G3DModel *model, goffset *n_bytes, guint32 level)
{
	...
	while(!g3d_stream_eof(stream) && (*n_bytes > 0)) {
		...
		*n_bytes -= 8;
		if(*n_bytes < 0)
			return FALSE;
		g_debug("\\%s0x%08x @ 0x%08x (%u bytes)", padding, id,
